...
class Object:
    ...
    def get_obj_vrelat(self,buf):
        self.vrelat = (((c_ushort(buf[5]).value & 0x3f) << 3) | ((c_ubyte(buf[6]).value >> 5) & 0x07)) * 0.25 - 64
    def get_obj_rcs(self,buf):
        self.rcs = c_ubyte(buf[7]).value * 0.5 - 64
    def get_obj_state(self,buf):
        self.state = (c_ubyte(buf[6]).value >> 2) & 0x07
    def get_obj_class(self,buf):
        self.type = (c_ubyte(buf[3]).value & 0x07)
    ...
    while 1:#一直循环查询接收。
        ...
        if ret > 0:#接收到数据
            for i in range(0,ret):
                msg = rx_vci_can_obj.STRUCT_ARRAY[i].Data
                ...
                if rx_vci_can_obj.STRUCT_ARRAY[i].ID == 0x60b:
                    obj = Object()
                    obj.get_obj_ID(msg)
                    obj.get_obj_distlat(msg)
                    obj.get_obj_distlong(msg)
                    obj.get_obj_vrelong(msg)
                    obj.get_obj_vrelat(msg)
                    obj.get_obj_rcs(msg)
                    obj_list.insert_object(obj)
                if rx_vci_can_obj.STRUCT_ARRAY[i].ID == 0x60c:
                    obj_id = c_ubyte(msg[0]).value
                    if obj_list.object_list[obj_id] != None:
                        obj_list.object_list[obj_id].get_obj_state(msg)
                if rx_vci_can_obj.STRUCT_ARRAY[i].ID == 0x60D:
                    obj_id= c_ubyte(rx_vci_can_obj.STRUCT_ARRAY[i].Data[0]).value
                    if obj_list.object_list[obj_id] != None:
                        obj_list.object_list[obj_id].get_obj_class(msg)
